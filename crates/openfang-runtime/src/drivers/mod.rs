...
fn provider_defaults(provider: &str) -> Option<ProviderDefaults> {
    match provider {
        ...
        "copilot-cli" => Some(ProviderDefaults {
            base_url: "",
            api_key_env: "",
            key_required: false,
        }),
...
pub fn create_driver(config: &DriverConfig) -> Result<Arc<dyn LlmDriver>, LlmError> {
    ...
    // GitHub Copilot CLI — subprocess-based (via gh copilot)
    if provider == "copilot-cli" {
        let cli_path = config.base_url.clone();
        return Ok(Arc::new(copilot_cli::CopilotCliDriver::new(
            cli_path,
            config.skip_permissions,
    ...
    Err(LlmError::Api {
        ...
        message: format!(
            "Unknown provider '{}'. Supported: anthropic, gemini, openai, groq, openrouter, \
             ...
             chutes, venice, nvidia, codex, claude-code, copilot-cli. Or set base_url for a custom OpenAI-compatible endpoint.",
            provider
        ),
    })
}
...
mod tests {
    ...
    fn test_known_providers_list() {
        ...
        assert!(providers.contains(&"copilot-cli"));
